...
It's no uncommon we've `Lombok` on project as well.<br>
...
The most commom is doing something like:
```java
@Table(name = "car")
class CarEntity {
  @NotNull
...
However, people obsecade by things like _avoid boilerplate_, _I love Lombok_, _let's make Java in a Functional way_, _immutability everywhere_, etc.<br>
...
Looks like an harmless code, until we _observe the details and see that_ below:

> **1st, we've a entity as record**
...
Builder brings a_ lot static methods_ plus an extra `toBuilder` method.

> **3rd, It make us break the contract**

On the example we have **all fields required**.<br>
But, using `builder()` we start to break the contract by defining only someof the fields, e.g:
...
It doesn't make any sense to the our **Domain**.<br>
Even all fields were not required, I still see that `CarEntity.builder().build();`<br>
Where, in place, we should create a no args contructor in a fashion way: `new CarEntity()`.<br>
Period.

I'm not against records or Lombok.<br>
I'm against bad pattern we bring to codebase whem put all these things together.
